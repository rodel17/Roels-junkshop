<?php 
	...
	if (isset($_SESSION['unique_id'])) {
		...
		$item_unique_id = mysqli_real_escape_string($conn, $_POST['item_unique_id']);
		...
		if(!empty($item_name) && !empty($item_details) && !empty($item_date) && !empty($item_price)){

							$item_status = 'onsite';
					 		//update users data inside the table
					 		$sql2 = mysqli_query($conn, "INSERT INTO items (item_unique_id, item_name, item_date, item_details, price, item_status)
					 			                 VALUES ({$item_unique_id},'{$item_name}', '{$item_date}', '{$item_details}', '{$item_price}', '{$item_status}')");
					 		if ($sql2) {
					 			echo "success";
					 		}else{
		...
		echo "All input field are required!";
			...
			$extensions = ['png', 'jpeg', 'jpg']; //all valid image extension and store them in array
				...
				if(move_uploaded_file($tmp_name, "../php/item-images/".$new_img_name)){ //if user upload img move to
					 														//successfully
					$sql1 = mysqli_query($conn, "INSERT INTO items SET item_img = '$new_img_name' WHERE item_id = $item_id");
				}
}
?>